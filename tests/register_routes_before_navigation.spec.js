...
const{installMockEventRoutes,mockEvents,findEventCardByTitle} =require('../LoginHelperFolder/MockEventRoutes.helper');
const {createBookingFromFilters} = require('../LoginHelperFolder/createBookingFromFilters');
...
test('mock events page', async ({ page }) => {
  ...
  await  await page.getByRole('link', { name: 'Browse Events →' }).click();
  ...
  for (const event of mockEvents) {
    ...
    const eventCard =page.locator('[data-testid="event-card"]').filter({hasText:event.title});
    await expect(eventCard.getByRole('heading',{name:event.title})).toBeVisible();
    ...
    const targetTitle = await eventCard.getByRole('heading').innerText()
     const { card, mockEvent } = await findEventCardByTitle(page,targetTitle,mockEvents)
     await expect(card.getByRole('heading', { name: mockEvent.title })).toHaveText(targetTitle);
    //Assert the live title World Tech Summit is not visible anymore
   await expect(page.getByRole('heading', {name: 'World Tech Summit'})).not.toBeVisible();
   //Assert each mocked card shows the expected price text and seat availability text from your mock dataset
   const mockedcardPriceText = await eventCard.locator('.text-lg.font-bold.text-indigo-700').innerText();
   const numericPrice = parseInt(mockedcardPriceText.replace('$', '').replace(',', '').trim())
    console.log(`Price: ${numericPrice}`)
    await expect(numericPrice).toBe(event.price)
    const mockedCardSeatAvailability = await eventCard.locator('//span[@class="text-xs font-semibold text-emerald-600"]').innerText()
   ...
   //Assert the title link and Book Now link inside each card point to /events/{mockedId} for that same mocked record
    ...
    await expect(eventCard.getByRole('link',{name:event.title})).toHaveAttribute('href',`/events/${event.id}`)
    await expect(eventCard.getByRole('link', { name: 'Book Now' })).toHaveAttribute('href', `/events/${String(event.id)}`);
 }
...
const  livefiltersagainstthemockeddataset = await createBookingFromFilters(page, { searchText:'world', category:'Conference', city:'Hyderabad' })
